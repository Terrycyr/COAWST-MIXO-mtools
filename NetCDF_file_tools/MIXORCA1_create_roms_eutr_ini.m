clear all;
init_file = 'WFS_2002_ini_bio.nc'; delete(init_file);
year = 2002;
ot_start = 0.0;
...
date_out = datenum(2002,1,1,0,0,0);
...
SIT = si;
% for i=1:N
%  ON(:,:,i) = don(:,:,i)/ONDP_r.*scale_distance;
% end
ON = don*1./ONDP_r;
OC = ON*5;
...
%flag = repmat(dep<40&mask==1,1,1,size(si,3));
%flag(410:end,:,:) = 0;
flag = 1;
PHYT1_roms = ones(size(si))*1e-5.*flag;
PHYT2_roms = ones(size(si))*3e-5.*flag;
