...
fn_his = './WFS_RCA_2005_his_00031.nc';
fn_dia = './WFS_RCA_2005_dia_00031.nc';
if(mixo_flag==1)
    var_his = {'salt','temp','SAL','BSI','SIT','DO','NO23','PO4T','NH4T','PHYT1','PHYT2',...
        'PHYT3','ZOO1','ZOO2','LDON','RDON','LPON','RPON','LDOC','RDOC','LPOC','RPOC',...
        'A_C','A_NC','A_PC','A_CHLC'};
else
...
mat_data = 'C:\Users\cheny\Desktop\EcoHAB\Cruise_data\FWC_kb_dat_0006.mat';
[kb_ini0,kb_lon,kb_lat,kb_dat] = gen_kb_ini(date_ini,date_ini+1,kb_bg,mat_data,'mean');
kb_ini0 = kb_ini0.*m_Ccell;
kb_ini0(kb_ini0<kb_bg*m_Ccell) = kb_bg*m_Ccell;
...
M_FC = M_C*MFC_INI;
MFNC = M_C*MFC_INI/M_CN;
MFPC = M_C*MFC_INI/M_CP;
M_FCHLC = M_C*MFC_INI/M_CCHL;
%Sediment
...
if(mixo_flag==1)
    ...
    ncwrite(init_file,'PHYT2',PHYT2);
end
ncwrite(init_file,'PHYT3',PHYT3_his);
...
if(mixo_flag==1)
    ...
    ncwrite(init_file,'M_NC',M_NC);
    ncwrite(init_file,'M_PC',M_PC);
    ncwrite(init_file,'M_CHLC',M_CHLC);
    ncwrite(init_file,'M_FC',M_FC);
    ncwrite(init_file,'MFNC',MFNC);
    ncwrite(init_file,'MFPC',MFPC);
    ncwrite(init_file,'M_FCHLC',M_FCHLC);
end
...
subplot(1,3,3);
m_proj('Mercator','lat',[24 32],'long',[-87.8 -80.2]);
m_contourf(lon,lat,log10(A_C_his(:,:,end)/a_Ccell),100,'linestyle','none');
caxis([6 8]);
hold on;
m_gshhs_h('patch',[.6 .6 .6]);
hold on;
