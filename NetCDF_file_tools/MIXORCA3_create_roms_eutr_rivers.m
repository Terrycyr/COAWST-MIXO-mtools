...
%   If it already exists it will be overwritten!!.
    forc_file='WFS_2002_river_bio.nc';
...
%   Are you entering a grid file name (1 = yes, 0 = no)? 
    ...
    if (get_grid)
      grid_file = '../Model_grid/ROMS_WFS_10river_grid_v11.nc';    %<-enter name of grid here
%
...
mavguriverID = netcdf.defVar(nc_forc,'river_M_AVGCU','float',[riverdimID s_rhodimID river_timedimID]);
...
mforc_filecriverID = netcdf.defVar(nc_forc,'river_MFNC','float',[riverdimID s_rhodimID river_timedimID]);
...
mfpcriverID = netcdf.defVar(nc_forc,'river_MFPC','float',[riverdimID s_rhodimID river_timedimID]);
...
TAriverID = netcdf.defVar(nc_forc,'river_TA','float',[riverdimID s_rhodimID river_timedimID]);
netcdf.putAtt(nc_forc,TAriverID,'long_name','TOTAL ALKALINITY');
netcdf.putAtt(nc_forc,TAriverID,'units','umol L-1');
netcdf.putAtt(nc_forc, TAriverID,'time','river_time');
netcdf.putAtt(nc_forc,TAriverID,'field','river_TA, scalar, series')

DICriverID = netcdf.defVar(nc_forc,'river_DIC','float',[riverdimID s_rhodimID river_timedimID]);
netcdf.putAtt(nc_forc,DICriverID,'long_name','DISSOLVED INORGANIC CARBON');
netcdf.putAtt(nc_forc,DICriverID,'units','umol L-1');
netcdf.putAtt(nc_forc, DICriverID,'time','river_time');
netcdf.putAtt(nc_forc,DICriverID,'field','river_DIC, scalar, series')

CACO3riverID = netcdf.defVar(nc_forc,'river_CACO3','float',[riverdimID s_rhodimID river_timedimID]);
netcdf.putAtt(nc_forc,CACO3riverID,'long_name','CALCIUM CARBONATE');
netcdf.putAtt(nc_forc,CACO3riverID,'units','mmol L-1');
netcdf.putAtt(nc_forc, CACO3riverID,'time','river_time');
netcdf.putAtt(nc_forc,CACO3riverID,'field','river_CACO3, scalar, series')
