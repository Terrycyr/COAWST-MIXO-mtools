...
gauge_factor = [1 1 1 1.2 1.2 1.2 1.2 1.2 1.2 1 1.2];
out_date = datenum(year,1,1):datenum(year,12,31,24,0,0);
...
forc_file=['WFS_',num2str(year),'_river.nc'];
...
if (get_grid)
    grid_file = '../Model_grid/ROMS_WFS_new.nc';    %<-enter name of grid herec
    %
...
for i=1:size(river_flow)
    river_flow(i,:) = river_flow(i,:)*gauge_factor(i);
end
...
%12) Time series of river mud and sand.
%
% mud.
...
if(NCS>0)
    ...
    for i=1:num_rivers
        for k=1:N
            if(sum(i==[8 9 10])>0)
                river_mud_01(i,k,:) = (abs(river_transport(i,:))*35.3147*0.0063+5.12).*mud01_frac*mgl2kgm3;
                river_mud_02(i,k,:) = (abs(river_transport(i,:))*35.3147*0.0063+5.12).*mud02_frac*mgl2kgm3;
            else
                river_mud_01(i,k,:) = river_tss(i,:).*mud01_frac*mgl2kgm3;
                river_mud_02(i,k,:) = river_tss(i,:).*mud02_frac*mgl2kgm3;
            end
        end
    end
end
