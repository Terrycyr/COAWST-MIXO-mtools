...
year = 2022;
nontidal_dataset_dir = './';
nontidal_dataset_source = 'GLBy0.08/expt_93.0';

load(strcat(nontidal_dataset_dir,'non_tidal_clm_',num2str(year),'_no_IAN.mat'));
origin_date = date_clm + datenum(year,1,1);
ldate = round(length(origin_date)/nfiles);

for i=1:nfiles
    eval(strcat('fn{',num2str(i),'} = ''WFS_',num2str(year),'_clm',num2str(i),'_no_IAN.nc'';'));
