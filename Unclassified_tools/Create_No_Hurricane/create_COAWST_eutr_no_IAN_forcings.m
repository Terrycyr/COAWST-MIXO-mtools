...
hurricane_lanfall = datenum(2022,9,28,20,35,00)-time_ref;
N = 21;
replace_len_before = 2;
replace_len_after = 3.5;

var_fname = {'Uwind','Vwind'};
var_replace = {'Uwind','Vwind'};
methods =   [    2       2];
...
for nsec = 1
    for ii=1:length(var_replace)
        ...
        forcing_fname_orig = ['WFS_2022_2023_',var_fname{ii},num2str(nsec),'_bio_mixo.nc'];
        forcing_fname_out = ['WFS_2022_2023_',var_fname{ii},num2str(nsec),'_bio_mixo_no_IAN.nc'];
        ...
        if(methods(ii)==1)
            ...
            for i = 1:size(forcing_var_orig,1)
                for j=1:size(forcing_var_orig,2)
                    ...
                    tmp = smooth(tmp,4);
                    forcing_var(i,j,:) = interp1(forcing_time2,tmp,forcing_time);
                end
            end
        elseif(methods(ii)==2)
        ...
        for i = 1:length(plt_i)
            ...
            set(gca,'xtick',[hurricane_lanfall-30:10:hurricane_lanfall+30],...
                'xticklabel',datestr([hurricane_lanfall-30:10:hurricane_lanfall+30]+time_ref,'mm/dd'));
        end
        set(gcf,'color','w');
