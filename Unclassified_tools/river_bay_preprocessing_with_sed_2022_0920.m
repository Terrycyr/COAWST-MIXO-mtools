...
if(skip_step(1)==0)
    ...
    save(fn_WA,'tss_mean','tss_dat','-append');
end
...
if(skip_step(6)==0)
gen_bay_nutrients_ini('../Cruise_data',['./bay_initials_',num2str(year),'_0920.xlsx']...
    ,strcat('bay_ini_',num2str(year),'_0920.mat')...
    ,grd,bay_mask,year,include_cruise,[],[],[],Ini_DON_method,bay_tss_flag);
end

%Step 7
%Read river sediment raw datac
if(skip_step(7)==0)
    [tss_dat,tss_mean] = pre_preocess_river_tss('River_tss.xlsx',n_river);
    save(fn_WA,'tss_mean','tss_dat','-append');
end