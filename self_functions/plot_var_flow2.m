function [ax] = plot_var_flow2(fig_path,v1,v2,c1,c2,var_name,name,psize,xlab,ylab,range, year)
...
%         end
        ...
        if(mod(k,psize(1)*psize(2))==0||i==size(v2,2))   
            outname = [fig_path name '_' num2str(year) '_' num2str(ceil(k2/(psize(1)*psize(2))))];
