function [ dat_out, dat_m ] = read_woa( fdir, b_lon, b_lat, b_mask, b_sc, b_h, time, vname)
%   Detailed explanation goes here
    ...
    for mon = 1:12
        ...
        dat_m(:,mon) = mean(dat_pick(:,:,i),1,'omitnan');
        %dat_m(:,mon) = smooth(tmp,0.2,'loess');

        for j=1:length(b_mask)
            dat_final(j,:,mon) = interp1(dep,dat_m(:,mon),b_sc(j,:).*b_h(j),'linear');
        end
    end
