...
!
      ...
      CHARACTER(LEN=5) :: str, str2, str3, str4
!
...
!     Define Dimensions
     ...
     .//TRIM(ADJUSTL(str3))//'_TRANS_'
     .//TRIM(ADJUSTL(str4))//'.nc'
...
!     Define variables
     ...
     .//TRIM(ADJUSTL(str3))//'_TRANS_'
     .//TRIM(ADJUSTL(str4))//'.nc'
          status=nf90_open(TRIM(ADJUSTL(filename)),nf90_write,ncid1(i))
          status=nf90_inq_varid(ncid1(i),'ocean_time',VarID1)
          status=nf90_inq_varid(ncid1(i),'temp_time',VarID11)
          status=nf90_inq_varid(ncid1(i),'salt_time',VarID12)
        ENDDO
        
        status=nf90_inq_varid(ncid1(1),'zeta',VarID2)     ! free surface
        status=nf90_inq_varid(ncid1(2),'ubar',VarID3)     !ubar-momentum
        status=nf90_inq_varid(ncid1(3),'vbar',VarID4)   !vbar-momentum
        status=nf90_inq_varid(ncid1(4),'u',VarID5)      ! u-momentum
        status=nf90_inq_varid(ncid1(5),'v',VarID6)      ! v-momentum
        status=nf90_inq_varid(ncid1(6),'omega',VarID7)  ! omega-momentum
        status=nf90_inq_varid(ncid1(7),'temp',VarID8)     ! temperature
        status=nf90_inq_varid(ncid1(8),'salt',VarID9)     ! salinity
        status=nf90_inq_varid(ncid1(9),'AKs',VarID10)    ! diffusivity
        status=nf90_inq_varid(ncid1(10),'Huon',VarID13)   ! u-mass transport
        status=nf90_inq_varid(ncid1(11),'Hvom',VarID14)   ! v-mass transport
      ENDIF
